...
#
# .. your code here ..
X = pd.read_csv('/Users/superpup/Documents/edxpython/Module6DT/Datasets/dataset-har-PUC-Rio-ugulino.csv', sep=None)
...
#
# .. your code here ..
map_label = {'Man':0, 'Woman':1}
X['gender'] = X['gender'].map(map_label)
...
#
# .. your code here ..
X['how_tall_in_meters'] = map(lambda x: x.replace(',','.'),X['how_tall_in_meters'])
X['body_mass_index'] = map(lambda x: x.replace(',','.'),X['body_mass_index'])
#
...
# .. your code here ..
#print X.describe()
X = X.drop(['user','class'],1)
X = X.apply(lambda X: pd.to_numeric(X, errors='raised'))
...
print "Score: ", round((modelscore*100), 3)
